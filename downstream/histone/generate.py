...
def build_output_path(temperature: float, top_p: float) -> Path:
    temp_str = str(temperature).replace(".", "p")
    top_p_str = str(top_p).replace(".", "p")
    return OUTPUT_DIR / f"histone_temp_{temp_str}_top_p_{top_p_str}.txt"
...
def main():
    ...
    for setting in GENERATION_SETTINGS:
        ...
        top_p = setting["top_p"]
        output_path = build_output_path(temperature, top_p)

        with open(output_path, "w", encoding="utf-8") as f:
            for item in HISTONE_ITEMS:
                prompt = build_prompt(item)

                for _ in tqdm(
                    range(NUM_SAMPLES_PER_PROMPT),
                    desc=f"Generating for {item} | temp={temperature}, top_p={top_p}",
                ):
